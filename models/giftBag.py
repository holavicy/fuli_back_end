...
class GiftBagModel(object):
    ...
    def edit_gift_bag_status(cls, gift_bag_id, status, staff_no):
        try:
            ...
            update_sql = f'update gifts set status = {status}, update_by = {staff_no} where id = {gift_bag_id}'
    ...
    def edit_gift_bag_info(cls, data):
        ...
        # 更新gifts表中的数据
        update_sql = "update gifts set name = '%s', limit_goods_num = '%d', update_by = '%s' where id = '%s'" % (data['name'], data['limitGoodsNum'], data['staffNo'], data['id'])
        cursor.execute(update_sql)

        # 根据gifts.id删除gifts_goods_relation表中的关系
        update_relation_sql = "update gift_goods_relation set status = 2, update_by = '%s' where gift_id = '%s'" % (data['staffNo'], data['id'])
